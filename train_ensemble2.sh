export CUDA_VISIBLE_DEVICES=2
for TASK_NAME in mrpc
do
for seed in 1 2 3 4 5
do
for lr in 1e-5 
do
for BS in 16
do
for epoch in 4
do

#init_path=gated_v3/$TASK_NAME/gatedinit_base_seed${seed}_BS${BS}_lr${lr}_initepoch${init_epoch}
old_path=mrpc/baseline_base_seed0_BS64_lr9e-5_epoch3
save_path=ensemble2/$TASK_NAME/bert-large_seed${seed}_BS${BS}_lr${lr}_epoch${epoch}
#ensemble2/mrpc/bert-base_seed3_BS32_lr4e-5_epoch5/predict_results_mrpc.txt

python run_glue_ensemble.py \
  --model_name_or_path bert-large-cased \
  --old_model_name_or_path ${old_path} \
  --task_name $TASK_NAME \
  --do_train \
  --do_predict \
  --max_seq_length 128 \
  --per_device_train_batch_size ${BS} \
  --learning_rate ${lr} \
  --num_train_epochs ${epoch} \
  --seed ${seed} \
  --save_steps 5000 \
  --output_dir ${save_path}



done
done
done
done
done
